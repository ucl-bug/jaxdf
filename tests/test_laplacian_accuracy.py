...
def get_filenames(
  ...
  good_files = [base_folder + f for f in all_files if f.startswith(incipit) and f.endswith(extension)]
  return good_files
...
FILENAMES = get_filenames("laplacian_FourierSeries_")
...
def test_gradient_accuracy_fourier_series(
  ...
  if make_plot:
    ...
    if len(test_data["step_size"]) == 1:
      ...
      plt.plot(y_pred.on_grid[...,0], marker='o')
      plt.legend()
      plt.savefig(TEST_DATA_PATH + "/last_test.png")
      plt.close()

      plt.plot(f1, label="true spectrum", marker='o')
      plt.plot(f2, label="predicted spectrum", marker='o')
      plt.legend()
      plt.savefig(TEST_DATA_PATH + "/last_test_spect.png")
      plt.close()
