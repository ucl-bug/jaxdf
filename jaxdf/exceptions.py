...
def check_fun_has_params(f: Callable):
  ...
  if 'params' in fun_spec.args:
    raise SignatureError(f"The argument 'params' must be a keyword argument in {f}, not a positional argument. Example: def evaluate(x, *, params): ...")
  if 'params' not in fun_spec.kwonlyargs:
    raise SignatureError(f"The argument 'params' must be a keyword argument in {f}. Example: def evaluate(x, *, params): ...")
