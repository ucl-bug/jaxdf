...
from jaxdf.core import Discretization, Field
from jaxdf import primitives as pr
...
class Arbitrary(Discretization):
    ...
    @staticmethod
    def laplacian(u, independent_params=True):
...
class GridBased(Linear):
    def __init__(self, domain, is_field_complex=True):
        self.domain = domain
        self.is_field_complex = is_field_complex
    ...
    def from_array(self, values: jnp.ndarray, name: str, expand_params=False):
        if expand_params:
...
class RealFourierSeries(FourierSeries):
    ...
    @staticmethod
    def laplacian(u):
        return pr.FFTLaplacian(real=True)(u)
